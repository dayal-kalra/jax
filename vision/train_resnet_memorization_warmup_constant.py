...
def create_learning_rate_fn(config):
    ...
    constant_steps = config.num_steps
    # cosine decay for the rest of the steps
    constant_fn = optax.constant_schedule(config.lr_trgt)
    schedule_fn = optax.join_schedules(schedules = [warmup_fn, constant_fn], boundaries = [config.warm_steps])
    return schedule_fn
...
# Optimization parameters
config.schedule_name = 'linear_constant'
