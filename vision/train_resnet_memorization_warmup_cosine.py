...
def create_learning_rate_fn(config):
    ...
    warmup_fn = optax.linear_schedule(init_value = config.lr_init, end_value = config.lr_trgt, transition_steps = config.warm_steps)
    # estimate the number of cosine decay steps
    cosine_steps = max(config.num_steps - config.warm_steps, 1)
    # cosine decay for the rest of the steps
    cosine_fn = optax.cosine_decay_schedule(init_value = config.lr_trgt, decay_steps = cosine_steps)
    schedule_fn = optax.join_schedules(schedules = [warmup_fn, cosine_fn], boundaries = [config.warm_steps])
    return schedule_fn

def create_train_state(config: argparse.ArgumentParser, batch: Tuple, learning_rate_fn):
    x, y = batch
    ...
    opt = optax.sgd(learning_rate_fn, config.momentum)
...
def train_and_evaluate(config: argparse.ArgumentParser, train_ds: Tuple, test_ds: Tuple, random_batch: Tuple):
    ...
    state, num_params = create_train_state(config, train_ds, learning_rate_fn)
    ...
    for step in range(config.num_steps):     
        ...
        if ((step+1) % config.random_steps == 0):
            ...
            result = jnp.array([state.step, learning_rate_fn(state.step), train_loss_step, train_accuracy_step, test_loss_step, test_accuracy_step, random_loss_step, random_accuracy_step, grad_norm_step])
    ...
    # save the data
    path = f'{save_dir}/dynamics_{config.dataset}_m{config.random_examples}_{config.model}_n{config.width}_w{config.widening_factor}_d{config.depth}_bias{config.use_bias}_{config.act_name}_I{config.init_seed}_{config.loss_name}_augment{config.augment}_{config.opt}_{config.schedule_name}_lr{config.lr_trgt:0.4f}_Twarm{config.warm_steps}_T{config.num_steps}_k{config.random_steps}_B{config.batch_size}_m{config.momentum}.tab'
...
# Optimization parameters
config.schedule_name = 'linear_cosine'
